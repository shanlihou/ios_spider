...
def _get_cili_url(soup):
    ...
    ajax_get_cili_url = const.BASEHTTPS + '/ajax/uncledatoolsbyajax.php?lang=zh'
...
def parser_content(html, is_save=False):
    ...
    genre = (i.text.strip() for i in genre_doc.find_next(
        'p').select('span')) if genre_doc else ''
    #genre =(i.text.strip() for i in soup.find('p', text="類別:").find_next('p').select('span')) if soup.find('p', text="類別:") else ''
    genre_text = ''
    for tex in genre:
        genre_text += '%s   ' % tex
    categories['type'] = genre_text
    ...
    if is_save:
        with open('magnet.html', 'wb') as fw:
            fw.write(magnet_html)
...
if __name__ == '__main__':
    ...
    if op == 0:
        with open('example.html', 'rb') as fr:
            content = fr.read()
            dic = parser_content(content, True)
            print(dic['jpg_src'])
    elif op == 1:
